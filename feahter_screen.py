...
class FeatherScreenHelper:
    ...
    def draw_toolbar(
        ...
        *,
        ...
        if self._last_status_bar == status_bar_data:
            return
    ...
    def print_caption(self, caption: str):
        if self._last_file_caption == caption:
            return
...
class FeatherScreen:
    def __init__(self, config):
        ...
        self.gcode.register_command(
            "FEATHER_PRINT_STATUS", self.cmd_FEATHER_PRINT_STATUS
        )
    ...
    def _init(self):
        ...
        self._toolbar_timer = self.reactor.register_timer(
            self._update_status_bar, self.reactor.NOW
        )
    ...
    def _change_state(self, new_state: ScreenState, eventtime):
        if self.state == ScreenState.DESTROYED:
            return
        ...
        if new_state in {ScreenState.PRINTING, ScreenState.PREPARING}:
            ...
            if stats_state == "complete":
                ...
                self.feather.print_left_panel(
                    self._convert_duration(stats["print_duration"], 2)
                )
